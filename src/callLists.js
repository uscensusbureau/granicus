...
const makeURL = (user_date, extURL, end, start) =>
  `${base_url}${extURL}?${makeURLDateRange(user_date, end, start)}`

const makeWklyURLArr = (user_date, extURL, ...days) =>
  days.map(day => makeURL(user_date, extURL, day, day + 6))
const makeWklyURLArrNudge = (user_date, str, ...days) =>
  days.map(day => makeURL(user_date, str, day - 1, day + 6))

const makeWkFnArr = (user_date, _topics, func, end, start) =>
  Object.values(_topics).map(id => makeURL(user_date, func(id), end, start))
...
const syntheticCallList = user_date =>
  interleaveWks(subscribersCallList(user_date), bulletinsCallList(user_date))
...
const topicsCallList = user_date => [
  topicS_1wk(user_date),
  topicS_2wk(user_date),
  topicS_3wk(user_date),
]
...
const makeTopicParams = topicIDs =>
  ''.concat(...Object.values(topicIDs).map(topicID => `&topic%5B%5D=${topicID}`))

const bulletinDetailsCallsForDays = (user_date, days) =>
